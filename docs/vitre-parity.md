...
| 2.6 | Source control. **2.6a done 2026-09-04** — git actions: header quick action + menu, commit dialog, default-branch confirm, vcs status stream, pull/init, staged progress toasts. **2.6b done 2026-09-04** — branch toolbar: ref picker (paged/searched/badged), switch/create with optimistic label, PR pill, locked workspace label. Remaining: PR-thread dialog, worktree cleanup | L+L+M+S | `BranchToolbar.tsx`, `GitActionsControl.tsx` (logic.ts is pure/portable), `PullRequestThreadDialog.tsx` |
