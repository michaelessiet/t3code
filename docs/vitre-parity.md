...
**Snapshot: 56 present · 43 partial · 92 absent.** M1 (chat core) and M2 slices 1–5
...
source control (git actions, branch toolbar, thread delete + worktree cleanup) and 2.7
diff review comments landed 2026-09-04.
...
| 2.5 | **Core done 2026-09-03** — Terminal: alacritty_terminal PTY-less grid renderer + attach/write/resize lifecycle, input arbitration, tabs/splits, theme sync. Satellites still open: links, selection→chat-context, running indicators, dock terminal surface content | XL core + M satellites | `ThreadTerminalDrawer.tsx`, `state/terminalSessions.ts`, `terminal-links.ts`, `terminalUiStateStore.ts` |
