...
**Snapshot: 41 present · 33 partial · 115 absent.** M1 (chat core) and M2 slices 1–5
...
knowledge graph, multi-environment).
...
| # | Slice | Size | Key Electron refs |
|---|---|---|---|
| 2.1 | Right-panel dock: multi-surface tab strip, per-thread persistence, close/cycle chords, resize + width persistence | XL+S | `rightPanelStore.ts`, `RightPanelTabs.tsx` |
...
| Feature | Electron ref | Vitre ref | Status | Size | Notes |
|---|---|---|---|---|---|
| Rebindable keybindings from server config (keybindings.json, when-expression contexts, superseded-default migration, settings UI) | `packages/shared/src/keybindings.ts` | `crates/vitre-app/src/main.rs` | ❌ absent | XL | Vitre hard-codes 7 chords in main.rs (mod+s, shift-alt-f, mod+p, mod+shift+f, mod+shift+p, mod+shift+o, mod+j) matching Electron defaults, with a code comment deferring rebindability to 'the keymap work'. Electron: server-resolved ResolvedKeybindingsConfig, when contexts (editorFocus/terminalFocus/fileTreeFocus/rightPanelFocus/modelPickerOpen), resolveShortcutCommand, migration table. Also unported chords: mod+n chat.new, mod+w closeSurface, ctrl+tab thread cycling, mod+shift+e fileTree.toggleFocus, f2 rename, mod+o openFavorite, mod+i showCompletions, mod+d diff.toggle. |
| Right panel tabbed surfaces (per-thread file/diff/search/plan/browser/terminal tabs; mod+w close, close-others/to-right/all, mod+shift+[ ] and ctrl+tab cycling, dirty dots, persisted per thread) | `apps/web/src/rightPanelStore.ts` | `crates/vitre-app/src/files.rs` | ❌ absent | XL | Vitre has a single FilesPanel with one open file and a header close button; no tab strip, no multiple open files, no per-thread surface persistence, no dirty-dot-per-tab (single header dot only). This is the biggest structural gap in the files/navigation area. |
...
| Files panel toggle (⌘J / rightPanel.toggle) with panel recreated per project | `apps/web/src/components/CommandPalette.tsx` | `crates/vitre-app/src/chat.rs` | ✅ present | S | ToggleFilesPanel bound to mod+j; ensure_files_panel recreates FilesPanel when the open thread's cwd changes; QuickSearch/palette file opens force the panel open (Electron reveals collapsed right panel the same way). Missing: mod+alt+b alias and explorer-open localStorage persistence (Vitre panel state resets per launch). |
...
| Feature | Electron ref | Vitre ref | Status | Size | Notes |
|---|---|---|---|---|---|
| Right-panel dock: multi-surface tab strip (plan/diff/files/file/browser/terminal/search/graph) | `apps/web/src/rightPanelStore.ts` | `crates/vitre-app/src/chat.rs` | 🟡 partial | L | Vitre has exactly one right surface — the files panel behind cmd-J (toggle_files). Electron: ordered surfaces per thread with persistence (t3code:right-panel-state:v2 version 10), activate/close/close-others/close-to-right, reconcile against live browser tabs and workspace availability; tab strip UI in RightPanelTabs.tsx (603 lines). |
| Right panel resize handle, width persistence, sheet mode on narrow windows | `apps/web/src/components/preview/RightPanelResizeHandle.tsx` | — | ❌ absent | S | rightPanelLayout.ts + threadSidebarWidth.ts + RightPanelSheet.tsx; gotcha from memory: resize-handle z-20 overlay eats flush-left gutter clicks. |
