...
use serde_json::json;
use vitre_rpc::{EnvironmentHttp, RpcSession, Sidecar, SidecarConfig, StreamEvent};
...
async fn main() -> anyhow::Result<()> {
    ...
    loop {
        match tokio::time::timeout(Duration::from_secs(10), subscription.next()).await {
            Ok(Some(StreamEvent::Values(values))) => {
                ...
                synchronized |= values
                    .iter()
                    .any(|value| value.get("kind").and_then(|kind| kind.as_str()) == Some("synchronized"));
                if chunks == 1 {
                    ...
                    match subscription.try_next() {
                        Some(_) => println!("[spike]     FAIL: server streamed past an un-Acked chunk"),
                        None => println!("[spike]     ack pacing holds (nothing arrived before Ack)"),
                    }
                }
                subscription.ack()?;
                if synchronized {
            ...
            Err(_) => {
                ...
                println!("[spike]     note: no synchronized marker within 10s (chunks so far: {chunks})");
                break;
            }
        }
    }
    ...
    match tokio::time::timeout(Duration::from_secs(5), subscription.next()).await {
        ...
        Ok(Some(StreamEvent::Values(_))) => println!("[spike]     interrupt: unexpected extra chunk"),
